...
rec_sum = {}
# 2) 내가 이미 좋아요한 타이틀
my_likes = df.loc[df['email'] == temail, 'file_title']

# 3) 비슷한 사용자들이 좋아한 타이틀 합산
# rec_sum = (
#     df.loc[(df['email'].isin(semail)) & ~(df['file_title'].isin(my_likes))]
#       .groupby('file_title')['like_count'].sum()
# )
rec_sum = df.loc[(df['email'].isin(semail)) & ~(df['file_title'].isin(my_likes))]
rec_sum2= rec_sum.groupby('file_title')['like_count'].sum()

print(rec_sum2)
print()

# 4) 내림차순 정렬 + 최대 2개 뽑기
top_N = rec_sum2.sort_values(ascending=False).head(2)
